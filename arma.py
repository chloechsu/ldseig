...
def _fit_arma_iter(outputs, inputs, p, q, r, l2_reg=0.0):
  ...
  if inputs is not None:
    ...
    lagged_in = np.concatenate(
        [lagmat(inputs[:, i], maxlag=r, trim='both') for i in range(in_dim)],
        axis=1)
...
def fit_arma_iter(outputs, inputs, guessed_dim, l2_reg=0.0):
  ...
  Returns:
    Fitted AR coefficients.
  """
  return _fit_arma_iter(outputs, inputs, p=guessed_dim, q=guessed_dim - 1,
          r=guessed_dim - 1, l2_reg=l2_reg)
...
def fit_arma_mle(outputs, inputs, guessed_dim, method='css-mle'):
  ...
  p = guessed_dim
  q = guessed_dim
  k = 1 # for the constant
  if inputs is not None:
    k = k.shape[1] + 1
  start_params = np.zeros(p+q+k)
  start_params[:p] = fit_ar(outputs, inputs, p)
  model = ARMA(outputs, order=(p, q), exog=inputs)
  arma_model = model.fit(start_params=start_params, trend='c', method=method,
      disp=0, transparams=False)
  return arma_model.arparams, arma_model.maparams
